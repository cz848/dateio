...
　　这是一个轻量级、无依赖的日期处理类库，用于日期的取值、格式化显示及计算等，主要针对于小程序环境使用（也可以用于其它环境）。
　　创建的初衷是想在小程序中显示日期时能方便，统一化处理。但是看到moment.js庞大的体积就让我望而却步。找到dayjs也觉得不够轻量，有些方法并不需要，浪费无谓的体积，所以就打算自己实现。
　　但是很多基础api还是参考了moment的设计，同时也借鉴了dayjs的做法，但是为了尽量简化调用，也为了与format的参数保持一致，基础api上还是做了很多改动，所以与两者也有很大的不同。
