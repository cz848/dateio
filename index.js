...
const toDate = input => {
  ...
  if (typeof input === 'string' && !/T.+(?:Z$)?/i.test(input)) {
    ...
    input = new Date(input.slice(0, 3).join('/')).setHours(...input.slice(3), 0);
  }
...
class DateIO {
  ...
  // 0...1542759768
  U(U) {
    return isDefined(U) ? this.init(U * 1e3) : Math.round(this / 1e3);
  }
  ...
  // 设置以上格式的日期
  set(unit, a, b, c, d) {
    return setUnitRegExp.test(unit) ? this[unit](a, b, c, d) : this;
  }
