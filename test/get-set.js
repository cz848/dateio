...
test('Get Unix timestamp', () => {
  ...
  expect(dateio().set('u', 0).valueOf()).toBe(moment(0).valueOf());
  expect(dateio().set('u', 1).valueOf()).toBe(moment(1).valueOf());
  expect(dateio().set('U', 1546).valueOf()).toBe(moment.unix(1546).valueOf());
  expect(dateio().set('U', 1546345).valueOf()).toBe(dateio(1546345000).U * 1000);
  expect(dateio.U(154633).valueOf()).toBe(moment.unix(154633).valueOf());
  expect(dateio.U(1546345789).valueOf()).toBe(dateio(1546345789000).get('U') * 1000);
});
...
test('Set Year', () => {
  ...
  expect(dateio().set('y', 0).valueOf()).toBe(moment().year(0).valueOf());
  expect(dateio().set('y', 2000).valueOf()).toBe(moment().year(2000).valueOf());
  expect(dateio().set('y', 2000, 4, 5).valueOf()).toBe(new Date().setFullYear(2000, 3, 5));
});

test('Set Month', () => {
  expect(dateio().set('m', 11).valueOf()).toBe(moment().set('month', 10).valueOf());
  expect(dateio().set('m', 1).valueOf()).toBe(moment().month(0).valueOf());
  expect(dateio().set('m', 2).valueOf()).toBe(new Date().setMonth(1));
  expect(dateio().set('m', 11, 31).valueOf()).toBe(new Date().setMonth(10, 31));
});

test('Set Day', () => {
  expect(dateio().set('d', 30).valueOf()).toBe(moment().set('date', 30).valueOf());
  expect(dateio().set('d', 0).valueOf()).toBe(moment().date(0).valueOf());
  expect(dateio().set('d', 1).valueOf()).toBe(moment().date(1).valueOf());
});

test('Set Hour', () => {
  expect(dateio().set('h', 6).valueOf()).toBe(moment().set('hour', 6).valueOf());
  expect(dateio().set('h', 0).valueOf()).toBe(moment().hour(0).valueOf());
  expect(dateio().set('h', 1).valueOf()).toBe(moment().hour(1).valueOf());
  expect(dateio().set('h', 2, 10, 10, 123).valueOf()).toBe(new Date().setHours(2, 10, 10, 123));
});

test('Set Minute', () => {
  expect(dateio().set('i', 59).valueOf()).toBe(moment().set('minute', 59).valueOf());
  expect(dateio().set('i', 0).valueOf()).toBe(moment().minute(0).valueOf());
  expect(dateio().set('i', 1).valueOf()).toBe(moment().minute(1).valueOf());
  expect(dateio().set('i', 10, 10, 123).valueOf()).toBe(new Date().setMinutes(10, 10, 123));
});

test('Set Second', () => {
  expect(dateio().set('s', 59).valueOf()).toBe(moment().set('second', 59).valueOf());
  expect(dateio().set('s', 0).valueOf()).toBe(moment().second(0).valueOf());
  expect(dateio().set('s', 1).valueOf()).toBe(moment().second(1).valueOf());
  expect(dateio().set('s', 10, 123).valueOf()).toBe(new Date().setSeconds(10, 123));
});

test('Set Millisecond', () => {
  expect(dateio().set('ms', 999).valueOf()).toBe(moment().set('millisecond', 999).valueOf());
  expect(dateio().set('ms', 0).valueOf()).toBe(moment().millisecond(0).valueOf());
  expect(dateio().set('ms', 1).valueOf()).toBe(moment().millisecond(1).valueOf());
  expect(dateio().set('ms', 1573908997062).valueOf()).toBe(moment().millisecond(1573908997062).valueOf());
  expect(dateio().set('ms', 1573908997062).valueOf()).toBe(dateio().set('ms', 0) + dateio().set('u', 1573908997062));
});
...
test('Set unknown things', () => {
  ...
  expect(dateio().set('d').valueOf()).toBe(moment().set('D', NaN).valueOf());
